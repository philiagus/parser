...
class ConvertArrayTest extends TestCase
{
    ...
    {
        $parser = (new ConvertArray())->withDefaultedKey('forced', 1);
    ...
    {
        $parser = (new ConvertArray())->withDefaultedKey($key, 'value');
        self::assertSame(
            [
                $key => 'value',
            ],
    ...
    public function testThatForcedKeysDoNotExceptNonIntegerOrString($key): void
    {
        ...
        (new ConvertArray())->withDefaultedKey($key, 'value');
    }
    ...
    public function testThatForcedKeysUseParser(): void
    {
        ...
        $parser = (new ConvertArray())->withDefaultedKey('key', 'value', $childParser);
        $result = $parser->parse([]);
        self::assertSame(
            [
                'key' => 'parsedValue',
    ...
    public function testThatForcedKeysUseParserEvenIfValueIsAlreadyPresent(): void
    {
        ...
        /** @var ParserContract $childParser */
        $childParser = $child->reveal();
        $parser = (new ConvertArray())->withDefaultedKey('key', 'will not be used', $childParser);
    ...
    {
        $parser = (new ConvertArray())->withKeyWhitelist(['key1', 2]);
        self::assertSame(
            [
                'key1' => 123,
                2 => 234,
    ...
    {
        $parser = (new ConvertArray())->withKeyWhitelist([$key]);
        self::assertSame(
            [
                $key => 'exists',
            ],
    ...
    {
        $this->expectException(ParserConfigurationException::class);
        (new ConvertArray())->withKeyWhitelist([$key]);
    }
    ...
    {
        $parser = (new ConvertArray())->withKeyWhitelist(['ignored', 'as well']);
    ...
    public function testThatForcedKeysMustExist(): void
    {
        ...
        $parser = (new ConvertArray())->withKey('key', $childParser);
        $this->expectException(ParsingException::class);
        $parser->parse([]);
    }
    ...
    public function testThatForcedKeysMustBeIntegerOrString($key): void
    {
        ...
        $this->expectException(ParserConfigurationException::class);
        (new ConvertArray())->withKey($key, $childParser);
    }
    ...
    public function testWithTypeExceptionMessage(): void
    {
        ...
        (new ConvertArray())->setTypeExceptionMessage($msg)->parse(false);
    }
    ...
    public function testWithKeyConvertingValueExceptionMessage(string $exception, string $expected, $key): void
    {
        ...
        (new ConvertArray())->withKey($key, (new ConvertArray()), $exception)->parse([]);
    }
